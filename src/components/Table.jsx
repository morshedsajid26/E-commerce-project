...
export function Table({ TableHeads, TableRows, headClass, tableClass, children, headers, wrapperClass = "overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm hide-scrollbar" }) {
            ...
            <thead>
              <tr>
                {headers.map((h, i) => {
                  ...
                  return (
                    <th key={i} style={{ width }} className={`text-center bg-slate-900 font-semibold text-white py-4 px-4 text-xs uppercase tracking-wider ${headClass}`}>
                      {label}
                    </th>
                  );
                })}
