...
export default function StorefrontClient({
  ...
  const handleRequestRestock = async (medId) => {
    if (!user) {
      return toast.error("Please Sign In from the top menu to request a restock");
    }
  ...
  const resetAuthForm = () => {
    // Left empty for compatibility if called
  };
    ...
    if (user.address) {
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          savedAddresses.push(user.address);
        }
      } catch (e) {
        savedAddresses.push(user.address);
      }
    ...
    return (
      <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-3">
        <div className="flex items-center gap-2 text-xs font-bold text-slate-500">
          ...
          <span className="text-slate-800 font-extrabold">{user.name}</span>
        </div>
      ...
      {mode === 'landing' && !searchQuery.trim() && (
        <>
          <PremiumHero 
            customer={user}
            onAuthClick={() => {
              // Now handled by header dropdown or redirect
              window.location.href = "/login";
            }}
          />
          <PremiumFeatures />
