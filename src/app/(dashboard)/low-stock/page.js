...
export default function LowStockPage() {
  const { lowStock, isLoading } = useProducts();
  const lowStockProducts = lowStock || [];
  
  const [selectedCompany, setSelectedCompany] = useState("All Companies");

  const companies = useMemo(() => {
    const allComp = lowStockProducts.map(m => m.company).filter(Boolean);
    return ["All Companies", ...Array.from(new Set(allComp))];
  }, [lowStockProducts]);

  const filteredProducts = useMemo(() => {
    if (selectedCompany === "All Companies") return lowStockProducts;
    return lowStockProducts.filter(m => m.company === selectedCompany);
  }, [lowStockProducts, selectedCompany]);
    ...
    <div className="space-y-6">
        ...
        <Card className="bg-amber-50 border-amber-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-amber-700 mt-1">There are {lowStockProducts.length} items below the safety stock level. We recommend creating a purchase order today.</p>
            </div>
          </div>
        </Card>
      </div>
            ...
            <Card title="Low Stock List">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4 pb-3 border-b border-slate-100">
                <p className="text-xs text-slate-500 font-medium">
                  Showing {filteredProducts.length} of {lowStockProducts.length} products
                </p>
      ...
      <div id="print-area" className="hidden print:block a4-width bg-white text-slate-900">
        <div className="text-center pb-6 border-b-2 border-slate-300 mb-6">
          <h1 className="text-2xl font-black uppercase tracking-wider text-slate-900">Gadgets BD</h1>
          <p className="text-xs text-slate-500 mt-1">Dhaka, Bangladesh | Phone: +880 1756-899699</p>
        ...
        <div className="mt-12 pt-6 border-t border-dashed border-slate-300 text-center text-[10px] text-slate-400">
          <p className="font-semibold text-slate-600">Gadgets BD Management System — Low Stock Report</p>
        </div>
      </div>
    </div>
  );
