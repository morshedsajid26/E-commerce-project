...
export default function PurchasesPage() {
  ...
  const defaultCompanies = ["Apple", "Samsung", "Xiaomi", "Anker"];
           ...
           onClick={() => {
              ...
              setPurchaseItems(
                 (row.items || []).map(item => ({
                    ...
                    category: item.product?.category || item.category || "Smartphone",
                    isNew: !item.productId,
  ...
  const addPurchaseItem = () => {
    setPurchaseItems([...purchaseItems, { productId: "", name: "", genericName: "", category: "Smartphone", isNew: false, quantity: 1, price: 0, sellingPrice: 0 }]);
  };
  ...
  const handlePurchase = async () => {
    ...
    // Validate selections
    if (purchaseItems.some(item => !item.productId && !item.name)) {
      toast.error("Please select a product or enter a name for all rows!");
      return;
    }
    ...
    try {
      const payload = {
        ...
        items: purchaseItems.map(item => ({
          productId: item.isNew || item.productId === "new" ? null : item.productId,
          name: item.name,
    ...
    <div className="space-y-6">
            ...
            <div className="border border-slate-100 rounded-xl overflow-visible bg-white">
               <Table 
                 headers={[
                   ...
                   { label: "Purchase Price", width: "15%" },
                   { label: "Selling Price", width: "15%" },
                   { label: "Total", width: "13%" },
                   { label: "Action", width: "5%" }
                 ]}
                 tableClass="table-fixed"
                 wrapperClass="overflow-visible rounded-xl border border-slate-200 bg-white shadow-sm"
               >
                        ...
                        {item.isNew ? (
                          <div className="relative flex items-center gap-2">
                             ...
                             <div className="w-[180px]">
                               <Dropdown 
                                 value={item.category || "Smartphone"}
                          ...
                          <div className="flex flex-col gap-2">
                            <Dropdown 
                              ...
                              options={(products || []).map(m => ({ label: m.name, value: m.id.toString() }))} 
                            />
                            <button 
                               onClick={() => updateItem(index, "isNew", true)}
                               className="text-[10px] text-blue-600 font-bold hover:underline self-start"
                             >
                               + Or Add New Product
                             </button>
                          </div>
                  ...
                  viewingPurchase.items.map((item, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/50">
                      <td className="px-4 py-3 font-medium text-slate-900">
                        {item.product?.name || products?.find(m => m.id === item.productId)?.name || 'New Product'}
                      </td>
                      <td className="px-4 py-3 text-center font-bold">{item.quantity}</td>
                      <td className="px-4 py-3 text-right text-slate-500">৳{item.unitPrice?.toFixed(2)}</td>
                      <td className="px-4 py-3 text-right font-medium text-medical-blue-600">৳{item.product?.sellingPrice?.toFixed(2) || '0.00'}</td>
                      <td className="px-4 py-3 text-right font-bold text-slate-900">৳{item.totalPrice?.toFixed(2)}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan={5} className="px-4 py-6 text-center text-slate-400 italic">No item details available for this record</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      >
        <p className="text-slate-600">
          Are you sure you want to delete this purchase from <span className="font-bold text-slate-900">{purchaseToDelete?.supplier}</span>? This will revert the stock for all included products. This action cannot be undone.
        </p>
      </Modal>
    </div>
  );
