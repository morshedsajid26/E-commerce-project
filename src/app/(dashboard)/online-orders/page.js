                  ...
                  {selectedOrder.items.map((item) => (
                    <div key={item.id} className="p-3 bg-white flex items-center justify-between text-sm">
                      <div className="min-w-0">
                        <div className="font-extrabold text-slate-800 truncate">{item.product.name}</div>
                        <div className="text-xs text-slate-400">{item.product.company} • {item.product.category}</div>
                      </div>
                    ...
                    if (isLowStock) {
                      return (
                        <div key={item.id} className="flex items-center gap-2 p-3 bg-red-50 text-red-700 rounded-xl border border-red-100 text-xs font-semibold">
                          <AlertCircle size={14} />
                          <span>Warning: {item.product.name} only has {item.product.stock} left in stock. (Ordered: {item.quantity})</span>
                        </div>
                      );
                    }
                    return null;
