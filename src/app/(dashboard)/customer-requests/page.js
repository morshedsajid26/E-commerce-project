...
import { getProductRequestsAdminAction } from "@/lib/actions/online-customer.actions";
...
export default function CustomerRequestsPage() {
  ...
  async function loadRequests() {
    setLoading(true);
    try {
      const data = await getProductRequestsAdminAction();
      setRequests(data || []);
    } catch (error) {
  ...
  const requestColumns = useMemo(() => [
    {
      ...
      render: (row) => (
        <div className="flex flex-col">
          <span className="font-bold text-slate-900">{row.product.name}</span>
          <span className="text-xs text-slate-500">{row.product.company} • {row.product.category}</span>
        </div>
      )
    },
    {
      ...
      Title: "Active Stock",
      render: (row) => (
        <span className={`font-bold ${row.product.stock < 2 ? "text-red-500" : "text-emerald-600"}`}>
          {row.product.stock} units
        </span>
      )
    },
    {
