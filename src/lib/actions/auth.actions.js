...
export async function loginAction(identifier, password) {
  try {
    // 1. Check if the user is an Admin/Staff (checking by email)
    const user = await prisma.user.findUnique({
      where: { email: identifier }
    });

    if (user && user.password === password) { // In a real app, use bcrypt
      const session = await encrypt({ id: user.id, identifier: user.email, role: user.role, type: "admin" });
      const expires = new Date(Date.now() + 2 * 60 * 60 * 1000);
      (await cookies()).set("session", session, { expires, httpOnly: true, path: "/" });
      return { success: true, user: { id: user.id, name: user.name, email: user.email, role: user.role, type: "admin" } };
    }

    // 2. Check if the user is a Customer (checking by phone)
    const customer = await prisma.onlineCustomer.findUnique({
      where: { phone: identifier }
    });

    if (customer && customer.password === password) {
      const session = await encrypt({ id: customer.id, identifier: customer.phone, role: "customer", type: "customer" });
      const expires = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000); // 7 days for customer
      (await cookies()).set("session", session, { expires, httpOnly: true, path: "/" });
      return { success: true, user: { id: customer.id, name: customer.name, phone: customer.phone, role: "customer", type: "customer" } };
    }
...
export async function updateUserAction(data) {
  try {
    ...
    if (session.type !== "admin") {
      throw new Error("Only admins can use this profile update action");
    }

    const { name, email, password, profilePicture } = data;

    // Check if email is already taken by another user
    if (email && email !== session.identifier) {
